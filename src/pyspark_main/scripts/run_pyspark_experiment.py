...
def run_training_models(spark: SparkSession):
    def process_data(partition_data):
        ...
        for row in partition_data:
            option2run = Option2Run(row.model, row.metric_to_opt, row.rank_prm, row.emb_size, row.lr, row.opt_name, row.batch_size, row.reg_coef_vectors, row.reg_coef_biases)
            top_main_for_option_run(None, None, 0, option2run)
            yield [row.model, row.metric_to_opt, row.rank_prm, row.emb_size, row.lr, row.opt_name, row.batch_size, row.reg_coef_vectors, row.reg_coef_biases, row.row_id, r]
