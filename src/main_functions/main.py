...
from src.torchfm.torch_utils.constants import epochs_num, default_base_filename, test_datasets_path, wrapper, auc
...
from src.torchfm.torch_utils.utils import set_torch_seed, get_device_str, get_dataloaders, get_datasets, BestError, \
    get_criterion, get_optimizer, EarlyStopper, get_model, print_msg


def train(model, optimizer, data_loader, criterion, device, option_to_run):
    model.train()
...
def train_wrapper(model, optimizer, data_loader, criterion, device, option_to_run):
    start = time.time()
    train(model, optimizer, data_loader, criterion, device, option_to_run)
    end = time.time()
    return end - start
...
def main(dataset_name, dataset_paths, option_to_run, epoch, criterion_name, weight_decay, device, study=None, trial=None):
    num_workers = 0
