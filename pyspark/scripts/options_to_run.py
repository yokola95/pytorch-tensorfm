import numpy as np
import itertools
from torchfm.torch_utils.constants import lowrank_fwfm, fwfm, pruned_fwfm, fm, logloss, auc, mse
...
class Option2Run:
    ...
    def __init__(self, m_to_check, met_to_opt, rank, emb_size, lr, opt_name, batch_size, return_l2, reg_coef_vectors, reg_coef_biases):
    ...
    def to_csv(self):
        ",".join([str(i) for i in [self.m_to_check, self.met_to_opt, self.rank, self.emb_size, self.lr, self.opt_name, self.batch_size, self.return_l2, self.reg_coef_vectors, self.reg_coef_biases]])
