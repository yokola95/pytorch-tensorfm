...
class WrapperMultivaluedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path, sep=',', secondary_sep='|', engine='c', header='infer'):
        ...
        self.targets = self.__preprocess_target(data['label'].to_numpy())
        data = data.drop('label', axis=1)

        data['genres'] = data['genres'].str.split(secondary_sep).apply(lambda x: [int(i) for i in x])

        self.max_length = max(data['genres'].map(len))

        num_columns = len(data.columns)
        global_offsets = list(range(num_columns - 1))

        self.weights = data['genres'].apply(lambda x: WrapperMultivaluedDataset._create_weights(x, self.max_length, num_columns - 1)).to_numpy()
        self.offsets = data['genres'].apply(lambda x: WrapperMultivaluedDataset._create_offsets(x, num_columns - 2, global_offsets)).to_numpy()

        data['genres'] = data['genres'].apply(lambda x: x + [0] * (self.max_length - len(x)))

        lst_genres = ['genre' + str(i) for i in range(0, 6)]
        df_genres = pd.DataFrame(data['genres'].to_list(), columns=lst_genres)

        self.indices = pd.concat([data.drop(['genres'], axis=1), df_genres], axis=1, join='inner').to_numpy()
    ...
    def __getitem__(self, index):
        return np.concatenate((self.indices[index], self.offsets[index], self.weights[index]), axis=0), self.targets[index]

    @staticmethod
    def _create_weights(lst, full_length, num_cols, align_val=0.0):
        length = len(lst)
        return [] if length == 0 else [1.0] * num_cols + [1.0 / length] * length + [align_val] * (full_length - length)
